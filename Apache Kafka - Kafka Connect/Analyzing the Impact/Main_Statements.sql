...
-- companies table
echo 'SELECT * FROM companies LIMIT 12;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER $POSTGRES_DB'
echo 'SELECT COUNT(company_id) FROM companies;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER $POSTGRES_DB'

-- economic_data table
echo 'SELECT * FROM economic_data LIMIT 12;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER $POSTGRES_DB'
echo 'SELECT COUNT(econ_data_id) FROM economic_data;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER $POSTGRES_DB'

-- purchases table
echo 'SELECT * FROM purchases LIMIT 8;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER $POSTGRES_DB'
echo 'SELECT COUNT(purchase_id) FROM purchases;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER $POSTGRES_DB'
...
CREATE SOURCE CONNECTOR IF NOT EXISTS pg_companies_table_source_conn WITH (
    'name'='source_connector_from_pg_companies_to_table',
    'connector.class'= 'io.confluent.connect.jdbc.JdbcSourceConnector',
    'connection.url'= 'jdbc:postgresql://postgres:5432/impact_analysis',
    'connection.user'= 'pg',
...
CREATE SOURCE CONNECTOR IF NOT EXISTS pg_econ_data_table_source_conn WITH (
    'name'='source_connector_from_pg_econ_data_to_table',
    'connector.class'= 'io.confluent.connect.jdbc.JdbcSourceConnector',
    'connection.url'= 'jdbc:postgresql://postgres:5432/impact_analysis',
    'connection.user'= 'pg',
    'connection.password'= 'pg',
...
CREATE SOURCE CONNECTOR IF NOT EXISTS pg_purchases_stream_source_conn WITH (
    'name'='source_connector_from_pg_purchases_to_stream',
    'connector.class'= 'io.confluent.connect.jdbc.JdbcSourceConnector',
    'connection.url'= 'jdbc:postgresql://postgres:5432/impact_analysis',
    'connection.user'= 'pg',
    'connection.password'= 'pg',
...
-- source connectors
DESCRIBE CONNECTOR pg_companies_table_source_conn;
DESCRIBE CONNECTOR pg_econ_data_table_source_conn;
DESCRIBE CONNECTOR pg_purchases_stream_source_conn;
...
-- companies
echo 'SELECT * FROM output_pg_companies LIMIT 12;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT(company_name) FROM output_pg_companies;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'

-- economic_data
echo 'SELECT * FROM output_pg_economic_data LIMIT 25;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT(econ_date) FROM output_pg_economic_data;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'

-- purchases
echo 'SELECT * FROM output_pg_purchases LIMIT 25;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT(purchases_date) FROM output_pg_purchases;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'

-- analyzing_purchases_and_company_data
echo 'SELECT * FROM "output_ANALYZING_PURCHASES_AND_COMPANY_DATA" LIMIT 25;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT("PURCHASE_DATE") FROM "output_ANALYZING_PURCHASES_AND_COMPANY_DATA";' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'

-- analyze_purchases_company_of_gaming_prod_category
echo 'SELECT * FROM "output_ANALYZE_PURCHASES_COMPANY_OF_GAMING_PROD_CATEGORY" LIMIT 25;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT("PURCHASE_DATE") FROM "output_ANALYZE_PURCHASES_COMPANY_OF_GAMING_PROD_CATEGORY";' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'

-- analyze_purchases_company_of_public_companies
echo 'SELECT * FROM "output_ANALYZE_PURCHASES_COMPANY_OF_PUBLIC_COMPANIES" LIMIT 25;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT("PURCHASE_DATE") FROM "output_ANALYZE_PURCHASES_COMPANY_OF_PUBLIC_COMPANIES";' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'

-- analyze_purchases_company_from_johnston_sons
echo 'SELECT * FROM "output_ANALYZE_PURCHASES_COMPANY_FROM_JOHNSTON_SONS" LIMIT 25;' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
echo 'SELECT COUNT("PURCHASE_DATE") FROM "output_ANALYZE_PURCHASES_COMPANY_FROM_JOHNSTON_SONS";' | docker exec -i postgres bash -c 'psql -U $POSTGRES_USER impact_analysis_target'
...
-- DROP statements
DROP CONNECTOR PG_PURCHASES_STREAM_SOURCE_CONN;
DROP CONNECTOR PG_ECON_DATA_TABLE_SOURCE_CONN;
DROP CONNECTOR PG_COMPANIES_TABLE_SOURCE_CONN;
