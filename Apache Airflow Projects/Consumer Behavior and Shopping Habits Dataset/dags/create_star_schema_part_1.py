...
def sub_dag_create_star_schema(table_name: str,
    ...
    for col in dim_tables_to_create:
        ...
        start_task = DummyOperator(
            task_id=f'start_of_create_{column_name}_dim_table',
            dag=sub_dag_create_dim,
        )

        create_dim_task = PostgresOperator(
            task_id=f'create_dim_{column_name}_table',
            sql="/sql/create_dim_table.sql",
        ...
        end_task = DummyOperator(
            task_id=f'end_of_create_{column_name}_dim_table',
            dag=sub_dag_create_dim,
        )
